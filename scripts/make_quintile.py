#!/usr/bin/env python
r"""Per-degree-quintile table: where a uniform dose does its damage, and where degree-adaptive weighting repairs it."""
import os, sys
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from per_user_significance import user_scores

ARMS = [('encoder', 'XSimGCLg_w00'), (r'\;$+$uniform', 'XSimGCLg_w10'),
        (r'\;$+$adaptive $\beta{=}0.5$', 'AdaG_b05'), (r'\;$+$adaptive $\beta{=}1$', 'AdaG_b10')]

print(r"""\begin{table}[t]
\centering
\caption{Where a \emph{uniform} dose does its damage, and where degree-adaptive weighting repairs it: NDCG@20 by user
...
regularization---and degree-adaptive weighting removes it monotonically, recovering $95\%$ of it in the top quintile.
